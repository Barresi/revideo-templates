import { renderVideo } from '@revideo/renderer'
import * as express from 'express'
import * as fs from 'fs'
import * as path from 'path'
import { v4 as uuidv4 } from 'uuid'
import { getWordTimestamps } from './utils/utils'

const PORT = parseInt(process.env.PORT) || 5000
const app = express()
app.use(express.json())

app.get('/', (req, res) => {
  res.status(200).send(`Hello World!`)
})

app.post('/render/top-bottom-temlate', async (req, res) => {
  try {
    const { variables } = req.body
    const jobId = uuidv4()

    console.log('Generating caption...')
    const words = await getWordTimestamps(variables.ugcVideoUrl)

    console.log('Rendering video...')
    await renderVideo({
      projectFile: './templates/TopBottomTemplate.ts',
      variables: { ugcVideoUrl: variables.ugcVideoUrl, imageUrls: variables.imageUrls, words: words },
      settings: { outFile: `${jobId}.mp4`, logProgress: true }
    })
    console.log('Finished rendering')

    const outputFilePath = path.join(process.cwd(), `./output/${jobId}.mp4`)

    if (fs.existsSync(outputFilePath)) {
      res.sendFile(outputFilePath) // alternatively (and recommended), upload file to a bucket
    } else {
      res.status(500).send('Rendered video not found')
    }
  } catch (err) {
    console.error('Error rendering video:', err)
    res.status(500).send('Error rendering video')
  }
})

app.listen(PORT, () => {
  console.log(`listening on port ${PORT}`)
})
