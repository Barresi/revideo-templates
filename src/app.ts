...
import { getWordTimestamps } from './utils/generatorCaption'
import { downloadMultipleFiles } from './utils/googleDrive'
...
app.post('/render/top-bottom-template', async (req, res) => {
  ...
  try {
    ...
    const imageResults = await downloadMultipleFiles(variables.imageUrls, jobDir.images)

    console.log(`[${jobId}] Downloading UGC video...`)
    const videoResults = await downloadMultipleFiles([variables.ugcVideoUrl], jobDir.videos)
    const ugcVideoResult = videoResults[0]

    console.log(`[${jobId}] Downloaded ${imageResults.length} images and 1 UGC video`)

    // Step 2: Extract audio from UGC video
    console.log(`[${jobId}] Extracting audio from UGC video...`)
    const audioResult = await extractAudioFromVideo(ugcVideoResult.filePath, jobDir.audio)
    ...
    const outputFilePath = path.join(jobDir.output, `result.mp4`)
    await renderVideo({
    ...
    scheduleCleanup(jobId, jobDir.root, 10)
